 ...
 */
function buildFlexzcartTextLayer({ w, h }) {
...
function buildTrSegment({ w, h, q = 80, f = "auto", crop, watermark = false }) {
  ...
  return `${base}:${buildFlexzcartTextLayer({ w, h })}`;
}
...
/**
 * Same optimizations as {@link imageKitOptimizedUrl} plus FLEXzcart text overlay (for share/download).
